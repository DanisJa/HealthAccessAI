        ...
        <div className="p-6">
          ...
          {role === "doctor" && (
            <p className="text-neutral-dark mb-4">
              You have <span className="font-semibold text-primary">{stats.appointments || 0} appointments</span> today 
              and <span className="font-semibold text-primary">{stats.reports || 0} pending reports</span> to review.
            </p>
          )}
          
          {role === "patient" && stats.nextAppointment && (
            <p className="text-neutral-dark mb-4">
              Your next appointment is on <span className="font-semibold text-primary">
                {formatDate(stats.nextAppointment)}
              </span> 
              {stats.doctor && (
                <> with <span className="font-semibold text-primary">{stats.doctor}</span></>
              )}.
            </p>
          )}
            ...
            {role === "doctor" && (
              <>
                ...
                <Button variant="outline" className="border-primary text-primary">
                  <span className="material-icons mr-1 text-sm">assignment</span> Patient Reports
                </Button>
              </>
            )}
